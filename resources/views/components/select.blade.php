...
@php
    ...
    // Base classes for the select wrapper
    $wrapperClasses = 'relative';

    // Base classes for the select field itself
    $selectClasses = 'appearance-none block w-full rounded-lg border-0 py-2.5 pr-10 shadow-[0_1px_2px_0_rgba(0,0,0,0.04)] dark:shadow-none ring-1 ring-inset sm:text-sm sm:leading-6 transition-all duration-150 hover:shadow-[0_0_0_2px] hover:shadow-accent/20';

    // Adjust padding if an icon is present
    $selectClasses .= $icon ? ' pl-10' : ' px-3';
...
<div class="w-full">
    ...
    <div class="relative mt-2">
        ...
        <select
            name="{{ $name }}"
            id="{{ $id }}"
            {{ $attributes->merge(['class' => "{$selectClasses} {$ringClasses}"]) }}
        >
            {{ $slot }}
        </select>
